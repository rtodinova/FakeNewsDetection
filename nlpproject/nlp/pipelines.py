...
def analize_alphas(feature_train, feature_test, y_train, y_test):
    ...
    for alpha in alphas:
        print('Alpha: ', alpha)
        tmp = train_and_predict(alpha, feature_train, feature_test, y_train, y_test)
        if tmp > max_score:
            max_score = tmp
            max_alpha = alpha
        print('Score: ', tmp)
...
def custom_pipeline_with_gensim(df):
    sentances = token.sentance_tokenize(df['text'])
    tokenized_docs = [token.word_tokenizer(doc.lower()) for doc in sentances]
